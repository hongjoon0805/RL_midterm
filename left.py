...
def start(mode, to_recv, to_send):
    model = DuelModel(12, 3, 21, 0.025)
    
    model.load_weights('models/2020-05-08_std_0.1_lr_0.000125_add_1_step_loss_no_tag.model')
    ...
    while True: 
        ...
        if obs == None:
            break
        obs = np.array(obs).reshape((-1,))[:12]
        actions = tf.math.reduce_sum(model(obs.reshape((1,12))) * support, axis=2)
